...
d= fread(snakemake@input[[1]], select= c('ID', 'MAF', 'BETA', 'SE', 'TOTALSAMPLESIZE'))

bw= fread(snakemake@input[[2]], select= c('ID', 'MAF', 'BETA', 'SE', 'TOTALSAMPLESIZE'))
...
z= mclapply(split(d, d$gene), format_eqtl, mc.cores= 3)
z= do.call('rbind', z)
fwrite(z, snakemake@output[[3]], sep= '\t', row.names=F, col.names= T)

z= mclapply(split(bw, bw$gene), format_eqtl, mc.cores= 3)
z= do.call('rbind', z)
