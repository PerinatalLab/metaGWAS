...
if (grepl('haplotype', snakemake@input[[1]])){
...
m1= lm(SVLEN_UL_DG~ moms + fets + dads + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + cohort + PARITY, d)

h= data.frame(summary(m1)$coefficients)[2:3, ]
names(h)= c('estimate', 'se', 'z', 'pvalue')
h$method= row.names(h)

ci= data.frame(confint(m1))[2:3, ]
names(ci)= c('lo95', 'up95')
ci$method= row.names(ci)
h= inner_join(h, ci, by= 'method')
h$outcome= 'UL'
...
m1= lm(SVLEN_UL_DG~ moms + fets + dads + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + cohort + PARITY, filter(d, spont== 1))

x= data.frame(summary(m1)$coefficients)[2:3, ]
names(x)= c('estimate', 'se', 'z', 'pvalue')
x$method= row.names(x)
